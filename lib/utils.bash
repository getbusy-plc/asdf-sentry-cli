...
GH_REPO="https://github.com/getsentry/sentry-cli"
TOOL_NAME="sentry-cli"
TOOL_TEST="sentry-cli --help"
...
download_release() {
	...
	if [ "${os_name}" == "darwin" ]; then
		url="$GH_REPO/releases/download/${version}/sentry-cli-${os_name}-${version}.tgz"
	else
		local arch
		arch="$(uname -m)"
		case "$arch" in
		...
		url="$GH_REPO/releases/download/${version}/sentry-cli-${os_name}-${arch}-${version}.tgz"
	fi
...
install_version() {
	...
	(
		...
		# TODO: Assert sentry-cli executable exists.
		local tool_cmd
